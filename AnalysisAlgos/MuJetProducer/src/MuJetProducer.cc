...
// Class:      MuJetProducer
// 
/**\class MuJetProducer MuJetProducer.cc LeptonJetProducers/MuJetProducer/src/MuJetProducer.cc
   ...
   , m_unionNumberAboveThresholdPt(iConfig.getParameter<double>("unionNumberAboveThresholdPt"))
{
   ...
   for (std::vector<edm::ParameterSet>::const_iterator detailedSelector = m_detailedSelectors.begin();  detailedSelector != m_detailedSelectors.end();  ++detailedSelector) {
      ...
      else throw cms::Exception("BadConfig") << "algorithm must be one of \"TMLastStation\", \"TM2DCompatibility\", \"TMOneStation\"" << std::endl;
      ...
      else throw cms::Exception("BadConfig") << "arbitrationType must be one of \"NoArbitration\", \"SegmentArbitration\", \"SegmentAndTrackArbitration\"" << std::endl;
   }
}
...
void
MuJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
//            fabs(one->eta()) < 2.4 &&  
//            one->isTrackerMuon() &&  
//            one->numberOfMatches(reco::Muon::SegmentAndTrackArbitration) >= 2 &&  
//            one->innerTrack()->hitPattern().numberOfValidTrackerHits() >= 8 &&  
//            one->innerTrack()->normalizedChi2() < 4. &&
//	    fabs(one->innerTrack()->dxy(vtx->position())) < 3. &&
//	    fabs(one->innerTrack()->dz(vtx->position())) < 30.) { 
...
//            fabs(two->eta()) < 2.4 &&
//            two->isTrackerMuon() &&
//            two->numberOfMatches(reco::Muon::SegmentAndTrackArbitration) >= 2 &&
//            two->innerTrack()->hitPattern().numberOfValidTrackerHits() >= 8 &&
//            two->innerTrack()->normalizedChi2() < 4.0 &&
//	    fabs(two->innerTrack()->dxy(vtx->position())) < 3. &&
//	    fabs(two->innerTrack()->dz(vtx->position())) < 30.) { 
	    ...
	    if (one != two  &&  muonOkay(*two)) {
	       ...
	       pat::MultiMuon muonPair(pairOfMuons, transientTrackBuilder_ptr, tracks_ptr, muons_ptr, caloTowers_ptr, m_centralTrackIsolationCone, m_unionTrackIsolationCone, m_centralTrackThresholdPt, m_unionTrackThresholdPt, m_centralCaloIsolationCone, m_unionCaloIsolationCone, m_centralNumberAboveThresholdCone, m_unionNumberAboveThresholdCone, m_centralNumberAboveThresholdPt, m_unionNumberAboveThresholdPt);
...
//            muon->innerTrack()->hitPattern().numberOfValidTrackerHits() >= 8 &&
//            muon->innerTrack()->normalizedChi2() < 4.0 &&
//	    fabs(muon->innerTrack()->dxy(vtx->position())) < 3. &&
//	    fabs(muon->innerTrack()->dz(vtx->position())) < 30.) {
      if (muonOkay(*muon)) {
	 bool isUsed = false;
   ...
   do {
	 ...
	 for (unsigned int j = i+1;  j < jets.size();  j++) {
	    ...
	    if (active[i]  &&  active[j]) {
		  ...
		  newjets.push_back(jets[i].merge(jets[j], transientTrackBuilder_ptr, tracks_ptr, muons_ptr, caloTowers_ptr, m_centralTrackIsolationCone, m_unionTrackIsolationCone, m_centralTrackThresholdPt, m_unionTrackThresholdPt, m_centralCaloIsolationCone, m_unionCaloIsolationCone, m_centralNumberAboveThresholdCone, m_unionNumberAboveThresholdCone, m_centralNumberAboveThresholdPt, m_unionNumberAboveThresholdPt));
		  active[i] = false;
		  active[j] = false;
	       }
	    }
